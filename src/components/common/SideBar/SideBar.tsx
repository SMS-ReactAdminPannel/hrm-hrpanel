        ...
        <nav className="flex flex-col">
          ...
          <div className="mb-2">
            <SidebarDropdown icon={<FiUsers />} label="Employee" isOpen={actualOpen}>
             <div
                className={`mb-2 relative -left-[12%] w-[112%]`}
              >
                <SidebarLink
                  to="/employee"
                  icon={<FiUsers />}
...
// SidebarDropdown Component with Blur Effect
  ...
  return (
    <div
      className={`w-full transition-all duration-300 ${
        expanded && isOpen ? "bg-white/15 backdrop-blur-lg rounded-2xl p-2 shadow-lg border border-white/20" : ""
      }`}
    >
      <button
        onClick={() => setExpanded(!expanded)}
        className={`flex items-center w-full py-2  mb-1  ${
          isOpen ? "justify-start gap-3 px-3 rounded-xl" : "justify-center px-0"
        } ${expanded && isOpen ? "" : ""}`}
      >
        <div className="text-lg text-white flex-shrink-0">{icon}</div>
        {isOpen && (
          <>
            ...
            <span className={`text-white text-sm transition-transform duration-200 ${expanded ? "rotate-180" : ""}`}>
              <FiChevronDown />
            </span>
          </>
        )}
      ...
      {expanded && isOpen && (
        <div className="ml-6 flex flex-col transition-all duration-300 animate-in slide-in-from-top-2">{children}</div>
      )}
    </div>
  )
}

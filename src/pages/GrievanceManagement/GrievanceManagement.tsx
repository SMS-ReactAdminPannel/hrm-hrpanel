...
const GrievanceManagement = () => {
  ...
  try {
    ...
    setGrievances((prev) =>
      prev.map((g:any) => (g._id === id ? { ...g, status: "solved" } : g))
    );
...
const fetchGrievances = async () => {
  try {
    ...
    console.log("API Response:", response);
    const data = response?.data ?? response; // Handle different API structures
    console.log("Data to set:", data); 
    setGrievances(Array.isArray(data) ? data : []);
  } catch (error) {
    console.error("API Error:", error); // Check if API failed
  }
};

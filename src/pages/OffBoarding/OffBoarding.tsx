...
import { 
  ...
  Plus, Search, Filter, Download, Mail, Phone, Building,
  ClipboardList, Key, CreditCard, Laptop, Shield, Users,
  MessageSquare, Star, ArrowRight, Eye, Edit, Trash2,
  ChevronDown, ChevronRight, Home, Briefcase
} from 'lucide-react';
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-lg border shadow-sm">
          ...
          <div className="space-y-4">
            {offboardingRequests.slice(0, 3).map(req => (
              <div key={req.id} className="border rounded-lg p-4 hover:bg-gray-50 cursor-pointer"
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{req.employee.department} • {req.exitDetails.type}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="flex items-center space-x-2">
                      <div className="w-16 bg-gray-200 rounded-full h-2">
                        <div className="bg-blue-600 h-2 rounded-full" style={{width: `${req.progress.percentage}%`}}></div>
                      </div>
      ...
      <div className="space-y-6">
            ...
            <div className="flex items-center space-x-6">
              ...
              <div>
                ...
                <div className="flex items-center space-x-4 mt-2 text-sm text-gray-500">
                  <span className="flex items-center"><Building className="w-4 h-4 mr-1" />{selectedEmployee.employee.department}</span>
                  <span className="flex items-center"><User className="w-4 h-4 mr-1" />ID: {selectedEmployee.employee.id}</span>
                  <span className="flex items-center"><Calendar className="w-4 h-4 mr-1" />Hired: {selectedEmployee.employee.hireDate}</span>
                </div>
              </div>
            </div>
            <div className="text-right">
              <span className={`px-3 py-1 rounded-full text-sm ${
                selectedEmployee.exitDetails.status === 'Completed' ? 'bg-green-100 text-green-800' :
                selectedEmployee.exitDetails.status === 'In Progress' ? 'bg-blue-100 text-blue-800' :
          ...
          <div className="bg-white p-6 rounded-lg border shadow-sm">
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-blue-600 mb-2">
                {selectedEmployee.progress.percentage}%
              </div>
              <div className="w-full bg-gray-200 rounded-full h-3 mb-3">
                <div className="bg-blue-600 h-3 rounded-full" style={{width: `${selectedEmployee.progress.percentage}%`}}></div>
              </div>
                ...
                {expandedChecklist[categoryIdx] && (
                  <div className="mt-3 space-y-2 ml-6">
                    {category.items.map((item, itemIdx) => (
                      <div key={itemIdx} className="flex items-center  p-2 hover:bg-gray-50 rounded">
                        <input 
                          type="checkbox" 
                          className="w-4 h-4 text-blue-600 rounded"
                          defaultChecked={Math.random() > 0.4}
                        />
                      <span className="text-sm ml-3">{item}</span>
                      <div className=' ml-auto'>
                        <input className="block mb-5 w-3/4 text-xs text-gray-900 border border-gray-300 rounded-lg cursor-pointer  focus:outline-none " accept='.pdf,.txt' id="small_size" type="file" />
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="flex justify-between">
          ...
          <div className="space-x-3">
            <button className="px-6 py-2 bg-[#006666] text-white rounded-lg ">
              Generate Reports
            </button>
          </div>
        </div>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <p className="text-gray-500">{req.employee.department} • {req.employee.position}</p>
                            <p className="text-sm text-gray-400">Exit Type: {req.exitDetails.type} | Last Day: {req.exitDetails.lastWorkingDay}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <span className={`px-3 py-1 rounded-full text-sm ${
                            req.exitDetails.status === 'Completed' ? 'bg-green-100 text-green-800' :
                            req.exitDetails.status === 'In Progress' ? 'bg-blue-100 text-blue-800' :
                            'bg-yellow-100 text-yellow-800'
                          }`}>
